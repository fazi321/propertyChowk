<template>
  <default-layout>
    <section class="container">
      ...
      <BlueHead :title="'Favorites'" />
      <div class="set-m" v-if="properties.length">
...
export default {
  ...
  methods: {
    update(v) {
      ...
      this.user.favorites = this.favorites;
      var filtered = this.filteredItems.filter((items) => {
        return v != items.docKey;
      });
      this.filteredItems = filtered;
    },
    filterProperty() {
      var { favorites } = this.user;
      if (favorites) {
        this.favorites = favorites;
        var fav = this.favorites;
        var filtered = this.$store.state.properties.filter(function (items) {
          return fav.includes(items.docKey);
        });
        this.filteredItems = filtered;
      }
    },
